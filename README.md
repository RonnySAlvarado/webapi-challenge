...
- [ ] Describe Middleware?
      It's a function that essentially intercepts/gets run on data that's being sent to the backend before it reaches the backend.
      Useful for checking for things like if an ID exists, etc.

- [ ] Describe a Resource?
      From the TK, everything is a resource when designing a RESTful web api. Each resource is accessible via a unique URL, resources can have multiple representations,
